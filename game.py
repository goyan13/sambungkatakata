    ...
    for p in room["players"]:
        try:
            msg = await context.bot.send_message(
                chat_id=p["chat_id"],
                text=f"⏱️ {current['name']}: {TURN_TIME}"
            )
            msg_ids.append((p["chat_id"], msg.message_id))
        except:
            pass

    room["timer_msgs"] = msg_ids

    # countdown
    async def countdown():
        for i in range(TURN_TIME - 1, 0, -1):
            await asyncio.sleep(1)

            for chat_id, msg_id in room.get("timer_msgs", []):
                try:
                    await context.bot.edit_message_text(
                        chat_id=chat_id,
                        message_id=msg_id,
                        text=f"⏱️ {current['name']}: {i}"
                    )
                except:
                    pass
